...
public class OrdersController {
	...
	public String del(@RequestParam("productId") int productid,
		...
		ordersservice.del(productid, date, state,session);
		
		return "forward:get1";
	}
	@RequestMapping(value="/del1",method=RequestMethod.GET)
	public String del1(@RequestParam("productId") int productid,
				@RequestParam("date") String date,
				@RequestParam("state") int state,
				HttpSession session
			){
	...
	public String xianshi(HttpSession session){
		Ordersdetails od = ordersservice.xianshi(session);
		if(od!=null){
	...
	public String queren(HttpSession session){
		Ordersdetails od = ordersservice.xianshi(session);
		if(od!=null){
			Ordersdetails od1 = new Ordersdetails();
			String date = (String)session.getAttribute("date");
			od1.setAddresss(od.getAddresss());
			od1.setUseraddress(od.getUseraddress());
			od1.setDate(date);
			od1.setConsignee(od.getConsignee());
			od1.setRegion(od.getRegion());
			od1.setPhone(od.getPhone());
			ordersservice.addshouhuo(od1);
		}
	...
	public String jian(@RequestParam("productId") int productId,
			@RequestParam("date") String date,
			@RequestParam("state") int state){
		ordersservice.jian(productId, date, state);		
		return "forward:get1";
	}
	@RequestMapping(value="/jia",method=RequestMethod.GET)
	public String jia(@RequestParam("productId") int productId,
			@RequestParam("date") String date,
			@RequestParam("state") int state){
		ordersservice.jia(productId, date, state);		
		return "forward:get1";
	}
